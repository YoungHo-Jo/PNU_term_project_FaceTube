...
int V4LWrapper_OutputSetup (fsl_v4l_out *display)
{
	...
	for (i = 0; i < display->g_output_num_buffers; i++)
	{
		...
		out_buffer[i].start = (unsigned char*)mmap (NULL, out_buffer[i].length,	PROT_READ | PROT_WRITE, MAP_SHARED, display->fd_output_v4l, out_buffer[i].offset);
...
int V4LWrapper_StartCapturing (fsl_v4l_cap *capture)
{
        ...
        for (i = 0; i < capture->g_capture_num_buffers; i++)
        {
                ...
                cap_buffer[i].start = (unsigned char*)mmap (NULL, cap_buffer[i].length, PROT_READ | PROT_WRITE, MAP_SHARED,capture->fd_capture_v4l, cap_buffer[i].offset);
